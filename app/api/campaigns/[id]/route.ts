  ...
  try {
    ...
    await execute(
      `UPDATE campaigns SET title=?, slug=?, subtitle=?, description=?, image_url=?, cta_label=?, cta_link=?, goal_amount=?, is_featured=?, is_active=?, sort_order=?, updated_at=NOW()
       WHERE id=?`,
      [title, slug, subtitle || null, description || null, image_url, cta_label || "Donate", cta_link || null, goal_amount || null, is_featured ? 1 : 0, is_active !== false ? 1 : 0, sort_order || 0, Number(id)]
    )
